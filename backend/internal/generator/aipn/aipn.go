// Package aipn สร้าง XML AIPN (IPD ประกันสังคม ม.33/39/40) → ส่ง cs8.chi.or.th
//
// แตกต่างจาก CIPN: ไม่มี AGENCY, DROPID/DRDX บังคับทุก PROC/DIAG,
// AN อาจใช้ "=" เป็น wildcard ใน re-submission.
package aipn
...
// Diag — DRDX = attribute doctor license (บังคับ SSO)
type Diag struct {
	XMLName xml.Name `xml:"DIAG"`
	DxType  string   `xml:"DXTYPE,attr"`
	DRDX    string   `xml:"DRDX,attr"`
	Code    string   `xml:",chardata"`
}

// Proc — DROPID บังคับ SSO
type Proc struct {
	XMLName xml.Name `xml:"PROC"`
	DROPID  string   `xml:"DROPID,attr"`
...
func Build(in Input) ([]byte, error) {
	if in.HCode == "" {
		return nil, fmt.Errorf("aipn: HCode required")
	}
	if in.HCodeSub == "" {
	...
	doc := Document{
		Header: Header{
			HospMain: in.HCode, HospSub: in.HCodeSub,
			Period: in.Period, SendDate: util.FormatDate(in.SendDate),
		},
	...
	if err != nil {
		return nil, fmt.Errorf("aipn marshal: %w", err)
	}
...
func mapAdmit(a model.IPDAdmit) Admit {
	...
	for _, dx := range a.Diagnoses {
		ad.Diagnoses = append(ad.Diagnoses, Diag{
			DxType: dx.Type, DRDX: dx.DoctorID, Code: dx.Code,
		})
	}
