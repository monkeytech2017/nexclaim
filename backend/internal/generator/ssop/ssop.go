// Package ssop สร้าง XML SSOP (OPD ประกันสังคม) → ส่ง cs8.chi.or.th
//
// แตกต่างจาก CSOP: ไม่มี AGENCY, DRDX/DROPID บังคับทุก DIAG/PROC.
package ssop
...
func Build(in Input) ([]byte, error) {
	if in.HCode == "" {
		return nil, fmt.Errorf("ssop: HCode required")
	}
	if in.HCodeSub == "" {
	...
	if err != nil {
		return nil, fmt.Errorf("ssop marshal: %w", err)
	}
...
func mapVisit(v model.OPDVisit) Visit {
	vi := Visit{
		HN: v.Patient.HN, PID: v.Patient.PersonID, SEQ: v.SEQ,
		DateOPD: util.FormatDate(v.DateOPD), TimeOPD: v.TimeOPD,
		Clinic: v.Clinic, TypeOut: v.TypeOut,
		UUC:   util.StrOr(v.UUC, "1"),
		Total: v.Total, Paid: v.Paid,
	}
	for _, dx := range v.Diagnoses {
		vi.Diagnoses = append(vi.Diagnoses, Diag{
